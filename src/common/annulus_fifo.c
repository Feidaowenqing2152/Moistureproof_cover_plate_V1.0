...
uint32_t fifo_write(_fifo_t *pfifo, const uint8_t *pbuf, uint32_t size)
{
	...
	{
		log_w("fifo write is null !!!!");
		return 0;
	}

    free_size = fifo_get_free_size(pfifo);
//    log_w("fifo_get_free_size = %d ",free_size);
    if(free_size == 0)
    {
    	log_w("fifo free size is zero !!!!");
        return 0;
    }
